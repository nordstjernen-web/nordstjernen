...
`cmake` is required to build the bundled lexbor subproject; install
the distro `cmake` package if it isn't already present.
...
The zip contains the `nordstjernen` binary, the
`data/compatibility-css/` runtime overrides, the application icon,
