...
3. **Config file** at `~/.config/nordstjernen/nordstjernen.conf`
   (Linux), `~/Library/Application Support/nordstjernen/nordstjernen.conf`
   (macOS), or `%APPDATA%\nordstjernen\nordstjernen.conf` (Windows):
...
passwords in the printed proxy URLs are masked. The
`about:nordstjernen` page also shows the proxy that would be used
for an HTTP and an HTTPS request.
