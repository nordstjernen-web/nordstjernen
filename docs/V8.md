...
console notice; `fetch`/`XMLHttpRequest` are absent; form controls
reflect attributes rather than live state; layout-derived metrics
(`offsetWidth`, `getBoundingClientRect`) report zero; WebGL, WebGPU,
the WebAssembly JS API, IndexedDB and WebExtensions are stubbed out of
the build.
...
2. **Forms & interaction** — live `value`/`checked` state, select and
   option activation, submit/reset through the embedder callbacks,
   focus integration, full key/mouse event fidelity.
