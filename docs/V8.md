...
console notice; `fetch`/`XMLHttpRequest` are absent; layout-derived
metrics (`offsetWidth`, `getBoundingClientRect`) report zero; WebGL,
...
3. **Window plumbing** — real `innerWidth`/`getBoundingClientRect`/
   `offsetWidth` from the layout tree (`ns_js_set_layout_root`),
   `getComputedStyle` over the style table, scroll APIs, `history` and
   the Navigation API.
4. **Async web APIs** — `fetch`/`XMLHttpRequest` over the async
   network stack, persistent `localStorage` (sqlite, shared with the
   QuickJS binding), timer nesting clamp and the eval budget watchdog.
