...
cp -r "$ROOT/data/compatibility-css" "$RPMTOP/SOURCES/"
...
Source1:        compatibility-css
Source2:        nordstjernen.svg
Source3:        nordstjernen.desktop
Source4:        README.md
Source5:        THIRD-PARTY-LICENSES.md
...
install -dm755 %{buildroot}%{_datadir}/nordstjernen/compatibility-css
...
cp -r %{SOURCE1}/. %{buildroot}%{_datadir}/nordstjernen/compatibility-css/
install -m644 %{SOURCE2} %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/nordstjernen.svg
install -m644 %{SOURCE3} %{buildroot}%{_datadir}/applications/nordstjernen.desktop
install -m644 %{SOURCE4} %{buildroot}%{_docdir}/nordstjernen/
install -m644 %{SOURCE5} %{buildroot}%{_docdir}/nordstjernen/
...
%changelog
* $(date "+%a %b %d %Y") Andreas Røsdal <andreas.rosdal@gmail.com> - ${VERSION}-1
- Prebuilt binary repackaged as an RPM.
SPEC_EOF

rpmbuild --define "_topdir $RPMTOP" -bb "$SPEC"

RPMFILE=$(find "$RPMTOP/RPMS" -name "nordstjernen-${VERSION}-*.rpm" | head -1)
if [ -z "$RPMFILE" ]; then
