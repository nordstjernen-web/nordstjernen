...
static void unlink_entry_pair(const char *meta_path, guint64 *running_total);
...
read_meta(const char *url, const char *meta_path)
{
    ...
    for (int i = 0; lines[i]; i++) {
        ...
        if      (klen == 9  && strncmp(line, "final_url",   9)  == 0) e->final_url    = g_strdup(v);
                 ...
                 strncmp(line, "cors_allow_origin", 17) == 0)
            e->cors_allow_origin = g_strdup(v);
...
nd_cache_get(const char *url, const char *partition)
{
    ...
    if (!e) return NULL;
    if (!is_cacheable_status(e->status)) {
        unlink_entry_pair(meta, NULL);
        nd_cache_entry_free(e);
        return NULL;
    }
    GStatBuf bst;
    if (g_stat(body, &bst) == 0 &&
        ((guint64)bst.st_size > G_MAXUINT ||
         (guint64)bst.st_size > cache_cap_bytes())) {
        nd_cache_entry_free(e);
        return NULL;
    }
    ...
    if (body_len > G_MAXUINT || body_len > cache_cap_bytes()) {
        nd_cache_entry_free(e);
        return NULL;
    }
           ...
           gint64 fetched_at)
{
    ...
    if (cors_allow_origin)
        append_meta_value(s, "cors_allow_origin", cors_allow_origin);
...
{
    if (!meta_path) return;
    gsize path_len = strlen(meta_path);
    const gsize suffix_len = strlen(".meta");
    if (path_len < suffix_len) return;
    gsize stem_len = path_len - suffix_len;
    char *stem = g_strndup(meta_path, stem_len);
    char *body = g_strconcat(stem, ".body", NULL);
    g_free(stem);
    GStatBuf st;
    if (running_total && g_stat(body, &st) == 0) {
        guint64 sz = (guint64)st.st_size;
        *running_total = (*running_total > sz) ? (*running_total - sz) : 0;
    }
    g_unlink(body);
