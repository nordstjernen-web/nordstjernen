...
static GKeyFile *g_engine_keyfile;
static gboolean  g_engine_keyfile_loaded;

static void
compat_engine_load(void)
{
    if (g_engine_keyfile_loaded) return;
    g_engine_keyfile_loaded = TRUE;
    char *path = compat_data_locate("html-engines.conf");
    if (!path) return;
    GKeyFile *kf = g_key_file_new();
    GError *err = NULL;
...
static gboolean
compat_engine_parse(const char *name, nd_html_engine *out)
{
    if (!name || !*name) return FALSE;
    if (g_ascii_strcasecmp(name, "lexbor") == 0) {
        if (!nd_html_engine_lexbor_available()) return FALSE;
        if (out) *out = ND_HTML_ENGINE_LEXBOR;
        return TRUE;
    }
    if (g_ascii_strcasecmp(name, "gumbo")   == 0 ||
        g_ascii_strcasecmp(name, "default") == 0) {
        if (out) *out = ND_HTML_ENGINE_GUMBO;
        return TRUE;
    }
    return FALSE;
}
...
gboolean
nd_compat_html_engine_for_host(const char *host, nd_html_engine *out)
{
    const compat_rule *r = compat_rule_for_host(host);
    if (!r) return FALSE;
    compat_engine_load();
    if (!g_engine_keyfile) return FALSE;
    char *val = g_key_file_get_string(g_engine_keyfile,
                                      "html-engines", r->id, NULL);
    if (!val) return FALSE;
    g_strstrip(val);
    gboolean ok = compat_engine_parse(val, out);
    g_free(val);
    return ok;
}

gboolean
nd_compat_html_engine_for_url(const char *url, nd_html_engine *out)
{
    if (!url) return FALSE;
    char *host = nd_url_host_from(url);
    gboolean ok = nd_compat_html_engine_for_host(host, out);
    g_free(host);
    return ok;
}
