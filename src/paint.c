...
paint_walk(cairo_t *cr, const nd_box *b, const char *highlight)
{
    ...
    cairo_operator_t blend = blend_mode_operator(b->style);
    const nd_css_value *mask_v = b->style ? b->style->values[ND_CSS_MASK_IMAGE] : NULL;
    ...
    if (!has_transform && !has_sticky) {
        const nd_css_value *posv = b->style ? b->style->values[ND_CSS_POSITION] : NULL;
    ...
    if (has_transform) {
        ...
        const nd_css_value *origin =
            b->style ? b->style->values[ND_CSS_TRANSFORM_ORIGIN] : NULL;
    ...
    if ((b->kind == ND_BOX_BLOCK || b->kind == ND_BOX_TABLE ||
        ...
        b->style && b->style->values[ND_CSS_CLIP_PATH] &&
        b->style->values[ND_CSS_CLIP_PATH]->kind == ND_CSS_V_KEYWORD &&
        b->style->values[ND_CSS_CLIP_PATH]->u.keyword &&
        strcmp(b->style->values[ND_CSS_CLIP_PATH]->u.keyword, "none") != 0) {
        cairo_save(cr);
